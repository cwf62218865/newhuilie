...
class mysql {
	...
	public function query($sql) {
        ...
        $QueryTime=$this->GetMicrotimeDiff5($StartTime);
        $this->querySQLList[]='执行时间：'.$QueryTime.'占用内存：'.$memory_usage.'执行语句：<br/>'.$this->sql;
        $this->queryTimeList[]=$QueryTime;
        $this->queryTimeCount+=$QueryTime;
        $this->queryCount++;
        if ($this->queryTime == ''){
            $this->queryTime = $this->GetMicrotime5();
        }
		return $this->result;
	}
